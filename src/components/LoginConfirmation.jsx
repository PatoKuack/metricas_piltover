...
const LoginConfirmation = ({summonerInfo, onClick}) => {
  return (
    <section className="flex flex-col place-items-center">
      ...
      <button type="button" onClick={onClick} className="w-fit mx-auto p-4 bg-teal-600 flex flex-col justify-center rounded md:p-8 sm:p-6">
        <img className="w-24 h-24 mb-2 border-2 border-double border-current rounded-full sm:w-32 sm:h-32" src={summonerInfo.iconURL} title="icono de usuario" alt="icono de usuario" />
        <p>Usuario: {summonerInfo.name}</p>
        <p className="self-center text-teal-100">Nivel: {summonerInfo.level}</p>
      </button>
    </section>
  )
}
